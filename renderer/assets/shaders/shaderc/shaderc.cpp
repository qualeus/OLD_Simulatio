...
const char* getName(ShadingLang::Enum _lang) { return s_shadingLangName[_lang]; }
...
static const Profile s_profiles[] = {
    {ShadingLang::ESSL, 100, "100_es"},       {ShadingLang::ESSL, 300, "300_es"},       {ShadingLang::ESSL, 310, "310_es"},       {ShadingLang::ESSL, 320, "320_es"},
    {ShadingLang::HLSL, 300, "s_3_0"},        {ShadingLang::HLSL, 400, "s_4_0"},        {ShadingLang::HLSL, 500, "s_5_0"},        {ShadingLang::Metal, 1000, "metal"},
    {ShadingLang::PSSL, 1000, "pssl"},        {ShadingLang::SpirV, 1311, "spirv13-11"}, {ShadingLang::SpirV, 1411, "spirv14-11"}, {ShadingLang::SpirV, 1512, "spirv15-12"},
    {ShadingLang::SpirV, 1010, "spirv10-10"}, {ShadingLang::SpirV, 1010, "spirv"},      {ShadingLang::GLSL, 120, "120"},          {ShadingLang::GLSL, 130, "130"},
    {ShadingLang::GLSL, 140, "140"},          {ShadingLang::GLSL, 150, "150"},          {ShadingLang::GLSL, 330, "330"},          {ShadingLang::GLSL, 400, "400"},
    {ShadingLang::GLSL, 410, "410"},          {ShadingLang::GLSL, 420, "420"},          {ShadingLang::GLSL, 430, "430"},          {ShadingLang::GLSL, 440, "440"},
};
...
static const char* s_unsignedVecs[] = {"uvec2", "uvec3", "uvec4", NULL};
...
static const char* s_allowedVertexShaderInputs[] = {"a_position",  "a_normal",    "a_tangent",   "a_bitangent", "a_color0",    "a_color1",    "a_color2",    "a_color3",
                                                    "a_indices",   "a_weight",    "a_texcoord0", "a_texcoord1", "a_texcoord2", "a_texcoord3", "a_texcoord4", "a_texcoord5",
                                                    "a_texcoord6", "a_texcoord7", "i_data0",     "i_data1",     "i_data2",     "i_data3",     "i_data4",     NULL};
...
void Options::dump() {
    BX_TRACE(
        ...
        ,
        shaderType, platform.c_str(), profile.c_str(), inputFilePath.c_str(), outputFilePath.c_str(), disasm ? "true" : "false", raw ? "true" : "false", preprocessOnly ? "true" : "false",
        depends ? "true" : "false", debugInformation ? "true" : "false", avoidFlowControl ? "true" : "false", noPreshader ? "true" : "false", partialPrecision ? "true" : "false",
        preferFlowControl ? "true" : "false", backwardsCompatibility ? "true" : "false", warningsAreErrors ? "true" : "false", keepIntermediate ? "true" : "false", optimize ? "true" : "false",
        optimizationLevel);

    for (size_t ii = 0; ii < includeDirs.size(); ++ii) { BX_TRACE("\t  include :%s\n", includeDirs[ii].c_str()); }

    for (size_t ii = 0; ii < defines.size(); ++ii) { BX_TRACE("\t  define :%s\n", defines[ii].c_str()); }

    for (size_t ii = 0; ii < dependencies.size(); ++ii) { BX_TRACE("\t  dependency :%s\n", dependencies[ii].c_str()); }
}
...
const char* getUniformTypeName(UniformType::Enum _enum) {
    ...
    if (idx < UniformType::Count) { return s_uniformTypeName[idx]; }
...
UniformType::Enum nameToUniformTypeEnum(const char* _name) {
    for (uint32_t ii = 0; ii < UniformType::Count * 2; ++ii) {
        if (NULL != s_uniformTypeName[ii] && 0 == bx::strCmp(_name, s_uniformTypeName[ii])) { return UniformType::Enum(ii / 2); }
    }
...
void strReplace(char* _str, const char* _find, const char* _replace) {
    ...
    for (int32_t ii = bx::strLen(replace); ii < len; ++ii) { replace[ii] = ' '; }
    replace[len] = '\0';

    BX_ASSERT(len >= bx::strLen(_replace), "");
    for (bx::StringView ptr = bx::strFind(_str, _find); !ptr.isEmpty(); ptr = bx::strFind(ptr.getPtr() + len, _find)) { bx::memCopy(const_cast<char*>(ptr.getPtr()), replace, len); }
}
...
struct Preprocessor {
    Preprocessor(const char* _filePath, bool _essl) : m_tagptr(m_tags), m_scratchPos(0), m_fgetsPos(0) {
        ...
        if (!_essl) { m_default = "#define lowp\n#define mediump\n#define highp\n"; }
    }
    ...
    static void fppError(void* /*_userData*/, char* _format, va_list _vargs) { bx::vprintf(_format, _vargs); }
...
uint32_t parseInOut(InOut& _inout, const bx::StringView& _str) {
    ...
    if (!str.isEmpty()) {
        ...
        do {
            ...
            if (delim.isEmpty()) { delim = bx::strFind(str, ' '); }
        ...
        murmur.begin();
        for (InOut::const_iterator it = _inout.begin(), itEnd = _inout.end(); it != itEnd; ++it) { murmur.add(it->c_str(), (uint32_t)it->size()); }
        hash = murmur.end();
    }
...
void help(const char* _error = NULL) {
    if (NULL != _error) { bx::printf("Error:\n%s\n\n", _error); }
...
bool compileShader(const char* _varying, const char* _comment, char* _shader, uint32_t _shaderLen, Options& _options, bx::FileWriter* _writer) {
    ...
    if ('\0' != profile_opt[0]) {
        ...
        for (profile_id = 0; profile_id < count; profile_id++) {
            if (0 == bx::strCmp(profile_opt, s_profiles[profile_id].name)) {
                break;
            } else if (s_profiles[profile_id].lang == ShadingLang::HLSL && 0 == bx::strCmp(&profile_opt[1], s_profiles[profile_id].name)) {
    ...
    for (size_t ii = 0; ii < _options.includeDirs.size(); ++ii) { preprocessor.addInclude(_options.includeDirs[ii].c_str()); }

    for (size_t ii = 0; ii < _options.defines.size(); ++ii) { preprocessor.setDefine(_options.defines[ii].c_str()); }

    for (size_t ii = 0; ii < _options.dependencies.size(); ++ii) { preprocessor.addDependency(_options.dependencies[ii].c_str()); }
    ...
    char glslDefine[128];
    if (profile->lang == ShadingLang::GLSL || profile->lang == ShadingLang::ESSL) { bx::snprintf(glslDefine, BX_COUNTOF(glslDefine), "BGFX_SHADER_LANGUAGE_GLSL=%d", profile->id); }

    char hlslDefine[128];
    if (profile->lang == ShadingLang::HLSL) { bx::snprintf(hlslDefine, BX_COUNTOF(hlslDefine), "BGFX_SHADER_LANGUAGE_HLSL=%d", profile->id); }
    ...
    if (0 == bx::strCmpI(platform, "android")) {
        ...
        if (profile->lang != ShadingLang::Metal) { preprocessor.setDefine(glslDefine); }
        char temp[256];
    ...
    switch (_options.shaderType) {
        case 'c': preprocessor.setDefine("BGFX_SHADER_TYPE_COMPUTE=1"); break;

        case 'f': preprocessor.setDefine("BGFX_SHADER_TYPE_FRAGMENT=1"); break;

        case 'v': preprocessor.setDefine("BGFX_SHADER_TYPE_VERTEX=1"); break;

        default: bx::printf("Unknown type: %c?!", _options.shaderType); return false;
    }
    ...
    while (!parse.isEmpty()) {
        ...
        if (eol.isEmpty()) { eol = bx::strFindEol(parse); }
        ...
        if (!eol.isEmpty()) {
            ...
            if (!typen.isEmpty() && !name.isEmpty() && !semantics.isEmpty()) {
                Varying var;
                if (!precision.isEmpty()) { var.m_precision.assign(precision.getPtr(), precision.getTerm()); }

                if (!interpolation.isEmpty()) { var.m_interpolation.assign(interpolation.getPtr(), interpolation.getTerm()); }
                ...
                if (profile->lang == ShadingLang::HLSL && profile->id < 400 && var.m_semantics == "BITANGENT") { var.m_semantics = "BINORMAL"; }

                if (!init.isEmpty()) { var.m_init.assign(init.getPtr(), init.getTerm()); }
    ...
    } else if ('c' == _options.shaderType)  // Compute
    {
        ...
        if (entry.isEmpty()) {
            ...
            if (profile->lang == ShadingLang::GLSL || profile->lang == ShadingLang::ESSL) {
                ...
                if (profile->lang != ShadingLang::PSSL) { preprocessor.writef(getPsslPreamble()); }
            ...
            if (preprocessor.run(input)) {
                if (_options.preprocessOnly) {
                    bx::write(_writer, preprocessor.m_preprocessed.c_str(), (int32_t)preprocessor.m_preprocessed.size(), &err);
            ...
            if (profile->lang == ShadingLang::GLSL || profile->lang == ShadingLang::ESSL) {
                ...
                *const_cast<char*>(entry.getPtr() + 4) = '_';
                ...
                if ('f' == _options.shaderType) {
                    ...
                    if (!insert.isEmpty()) { insert = strInsert(const_cast<char*>(insert.getPtr() + 1), "\nvec4 bgfx_VoidFrag = vec4_splat(0.0);\n"); }
                    ...
                    if (0 == numFragData) {
                        ...
                        if (!insert.isEmpty() && profile->id < 400 && !hasFragColor) { insert = strInsert(const_cast<char*>(insert.getPtr() + 1), "\ngl_FragColor = bgfx_VoidFrag;\n"); }
                    }
                    ...
                    for (InOut::const_iterator it = shaderInputs.begin(), itEnd = shaderInputs.end(); it != itEnd; ++it) {
                        ...
                        if (varyingIt != varyingMap.end()) {
                            const Varying& var = varyingIt->second;
                            preprocessor.writef(" \\\n\t%s%s %s %s : %s", arg++ > 0 ? ", " : "  ", interpolationDx11(var.m_interpolation.c_str()), var.m_type.c_str(), var.m_name.c_str(),
                    ...
                    for (uint32_t ii = 0; ii < BX_COUNTOF(hasFragData); ++ii) {
                        if (ii < maxRT) {
                            if (hasFragData[ii]) { addFragData(preprocessor, input, ii, arg++ > 0); }
                        } else {
                    ...
                    if (hasFrontFacing) {
                        if (profile->id < 400) { preprocessor.writef("#define gl_FrontFacing (__vface >= 0.0)\n"); }
                    }
                } else if ('v' == _options.shaderType) {
                    ...
                    if (!brace.isEmpty()) {
                        ...
                        if (!block.isEmpty()) { strInsert(const_cast<char*>(block.getTerm() - 1), "__RETURN__;\n"); }
                    }
                    ...
                    for (InOut::const_iterator it = shaderOutputs.begin(), itEnd = shaderOutputs.end(); it != itEnd; ++it) {
                        ...
                        if (varyingIt != varyingMap.end()) {
                            const Varying& var = varyingIt->second;
                            preprocessor.writef(" \\\n\t%s", var.m_name.c_str());
                            if (!var.m_init.empty()) { preprocessor.writef(" = %s", var.m_init.c_str()); }
                            preprocessor.writef(";");
                        }
                    }
            ...
            if (preprocessor.run(input)) {
...

                {
                    ...
                    if (profile->lang == ShadingLang::GLSL || profile->lang == ShadingLang::ESSL) {
                        ...
                        if (glsl_profile < 400) {
                            const bool usesTextureLod =
                                false || !bx::findIdentifierMatch(input, s_ARB_shader_texture_lod).isEmpty() || !bx::findIdentifierMatch(input, s_EXT_shader_texture_lod).isEmpty();
                            ...
                            if (profile->lang != ShadingLang::ESSL) {
                                const bool need130 =
                                    (120 == glsl_profile && (false || !bx::findIdentifierMatch(input, s_130).isEmpty() || usesInterpolationQualifiers || usesTexelFetch || usesUnsignedVecs));

                                bx::stringPrintf(code, "#version %d\n", need130 ? 130 : glsl_profile);
                                glsl_profile = 130;

                                if (need130) { bx::stringPrintf(code, "#define varying %s\n", 'f' == _options.shaderType ? "in" : "out"); }

                                if (usesInstanceID) { bx::stringPrintf(code, "#extension GL_ARB_draw_instanced : enable\n"); }
                                ...
                                if ((glsl_profile < 300) && usesUnsignedVecs) { glsl_profile = 300; }
                                ...
                                if (!bx::findIdentifierMatch(input, s_OES_standard_derivatives).isEmpty()) { bx::stringPrintf(code, "#extension GL_OES_standard_derivatives : enable\n"); }

                                if (!bx::findIdentifierMatch(input, s_OES_texture_3D).isEmpty()) { bx::stringPrintf(code, "#extension GL_OES_texture_3D : enable\n"); }
                        ...
                        if ((profile->lang == ShadingLang::GLSL && glsl_profile > 400) || (profile->lang == ShadingLang::ESSL && glsl_profile > 300)) {
                            ...
                            if (profile->lang == ShadingLang::ESSL) { glsl_profile |= 0x80000000; }
...
int compileShader(int _argc, const char* _argv[]) {
    ...
    if (NULL != profile) { options.profile = profile; }
    ...
    if (compiled) { return bx::kExitSuccess; }
...
int main(int _argc, const char* _argv[]) { return bgfx::compileShader(_argc, _argv); }