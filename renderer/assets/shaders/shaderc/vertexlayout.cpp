...
static const char* s_attrName[] = {
    "P",  "Attrib::Position",  "N",  "Attrib::Normal",    "T",  "Attrib::Tangent",   "B",  "Attrib::Bitangent", "C0", "Attrib::Color0",    "C1", "Attrib::Color1",
    "C2", "Attrib::Color2",    "C3", "Attrib::Color3",    "I",  "Attrib::Indices",   "W",  "Attrib::Weights",   "T0", "Attrib::TexCoord0", "T1", "Attrib::TexCoord1",
    "T2", "Attrib::TexCoord2", "T3", "Attrib::TexCoord3", "T4", "Attrib::TexCoord4", "T5", "Attrib::TexCoord5", "T6", "Attrib::TexCoord6", "T7", "Attrib::TexCoord7",
};
BX_STATIC_ASSERT(BX_COUNTOF(s_attrName) == Attrib::Count * 2);

const char* getAttribNameShort(Attrib::Enum _attr) { return s_attrName[_attr * 2 + 0]; }

const char* getAttribName(Attrib::Enum _attr) { return s_attrName[_attr * 2 + 1]; }
...
static AttribToId s_attribToId[] = {
    ...
    {Attrib::Position, 0x0001},  {Attrib::Normal, 0x0002},    {Attrib::Tangent, 0x0003},   {Attrib::Bitangent, 0x0004}, {Attrib::Color0, 0x0005},    {Attrib::Color1, 0x0006},
    {Attrib::Color2, 0x0018},    {Attrib::Color3, 0x0019},    {Attrib::Indices, 0x000e},   {Attrib::Weight, 0x000f},    {Attrib::TexCoord0, 0x0010}, {Attrib::TexCoord1, 0x0011},
    {Attrib::TexCoord2, 0x0012}, {Attrib::TexCoord3, 0x0013}, {Attrib::TexCoord4, 0x0014}, {Attrib::TexCoord5, 0x0015}, {Attrib::TexCoord6, 0x0016}, {Attrib::TexCoord7, 0x0017},
};
...
Attrib::Enum idToAttrib(uint16_t id) {
    for (uint32_t ii = 0; ii < BX_COUNTOF(s_attribToId); ++ii) {
        if (s_attribToId[ii].id == id) { return s_attribToId[ii].attr; }
    }

    return Attrib::Count;
}

uint16_t attribToId(Attrib::Enum _attr) { return s_attribToId[_attr].id; }
...
static AttribTypeToId s_attribTypeToId[] = {
    ...
    // added.
    {AttribType::Uint8, 0x0001}, {AttribType::Uint10, 0x0005}, {AttribType::Int16, 0x0002}, {AttribType::Half, 0x0003}, {AttribType::Float, 0x0004},
};
...
AttribType::Enum idToAttribType(uint16_t id) {
    for (uint32_t ii = 0; ii < BX_COUNTOF(s_attribTypeToId); ++ii) {
        if (s_attribTypeToId[ii].id == id) { return s_attribTypeToId[ii].type; }
    }

    return AttribType::Count;
}

uint16_t attribTypeToId(AttribType::Enum _attr) { return s_attribTypeToId[_attr].id; }
...
int32_t write(bx::WriterI* _writer, const VertexLayout& _layout, bx::Error* _err) {
    ...
    for (uint32_t attr = 0; attr < Attrib::Count; ++attr) { numAttrs += UINT16_MAX == _layout.m_attributes[attr] ? 0 : 1; }
...
int32_t read(bx::ReaderI* _reader, VertexLayout& _layout, bx::Error* _err) {
    ...
    for (uint32_t ii = 0; ii < numAttrs; ++ii) {
        ...
        if (Attrib::Count != attr && AttribType::Count != type) {
...
void vertexPack(const float _input[4], bool _inputNormalized, Attrib::Enum _attr, const VertexLayout& _layout, void* _data, uint32_t _index) {
    ...
    switch (type) {
            ...
            if (_inputNormalized) {
                if (asInt) {
                    switch (num) {
                        ...
                        case 2:
                            packed <<= 10;
                            packed |= uint32_t(*_input++ * 511.0f + 512.0f);
                            BX_FALLTHROUGH;
                    ...
                    switch (num) {
                        ...
                        case 2:
                            packed <<= 10;
                            packed |= uint32_t(*_input++ * 1023.0f);
                            BX_FALLTHROUGH;
        ...
        case AttribType::Float: bx::memCopy(data, _input, num * sizeof(float)); break;
    }
}
        ...
        case AttribType::Uint10: {
            ...
            if (asInt) {
                switch (num) {
                    default: BX_FALLTHROUGH;
                    case 3:
                        *_output++ = (float(packed & 0x3ff) - 512.0f) * 1.0f / 511.0f;
                        packed >>= 10;
                        BX_FALLTHROUGH;
                    case 2:
                        *_output++ = (float(packed & 0x3ff) - 512.0f) * 1.0f / 511.0f;
                ...
                switch (num) {
                    default: BX_FALLTHROUGH;
                    case 3:
                        *_output++ = float(packed & 0x3ff) * 1.0f / 1023.0f;
                        packed >>= 10;
                        BX_FALLTHROUGH;
                    case 2:
                        *_output++ = float(packed & 0x3ff) * 1.0f / 1023.0f;
...
static IndexT weldVerticesRef(IndexT* _output, const VertexLayout& _layout, const void* _data, uint32_t _num, float _epsilon) {
    ...
    for (uint32_t ii = 0; ii < _num; ++ii) {
        if (IndexT(-1) != _output[ii]) { continue; }
        ...
        for (uint32_t jj = 0; jj < _num; ++jj) {
            if (IndexT(-1) != _output[jj]) { continue; }

            float test[4];
            vertexUnpack(test, Attrib::Position, _layout, _data, jj);

            if (sqLength(test, pos) < epsilonSq) { _output[jj] = IndexT(ii); }
        }
    }
...
uint32_t weldVertices(void* _output, const VertexLayout& _layout, const void* _data, uint32_t _num, bool _index32, float _epsilon, bx::AllocatorI* _allocator) {
    if (_index32) { return weldVertices((uint32_t*)_output, _layout, _data, _num, _epsilon, _allocator); }
