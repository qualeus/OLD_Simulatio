...
void GuiManager::DrawGuiTimeBar() {
    ...
    time_bar_size = ImGui::AnimationLinear(show_gui["time_bar"], time_bar_size, 0.0f, G_TIME_BAR_SIZE, G_ANIMATION_SPEED);

    const float diff_size = G_TIME_BAR_SIZE - time_bar_size;

    ImGui::SetNextWindowPos(ImVec2(viewport->GetCenter().x - center_windows, viewport->WorkPos.y - diff_size));
    ...
    if (ImGui::Begin(
        ...
        if (ImGui::IsItemActive()) {
            ...
            // TODO: this->system->set_dt(this->system->get_dt() - dt_step);
        }
        ImGui::SameLine(0.0f, 0.0f);
        if (ImGui::Button(ICON_FK_BACKWARD, buttons_size)) {
            this->system->Step();  // TODO
        }
        ImGui::SameLine(0.0f, 0.0f);
    ...
    ImGui::SetNextWindowPos(ImVec2(viewport->GetCenter().x - center_handle, viewport->WorkPos.y + buttons_size.y - diff_size));
    ImGui::SetNextWindowSize(handle_size);

    if (ImGui::Begin(
            "##TimeWindowHandle", &always_show,
            ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoDocking)) {
        if (ImGui::Button(show_gui["time_bar"] ? ICON_FK_CARET_UP : ICON_FK_CARET_DOWN, handle_size)) { show_gui["time_bar"] = !show_gui["time_bar"]; }
        ImGui::End();
    }
    ImGui::PopStyleVar(3);
}
