...
void GuiManager::DrawGuiSideBar() {
    ...
    side_bar_size = ImGui::AnimationLinear(show_gui["side_bar"], side_bar_size, 0.0f, G_SIDE_BAR_SIZE, G_ANIMATION_SPEED);
    ...
    ImGui::SetNextWindowPos(ImVec2(viewport->WorkPos.x - diff_size, viewport->WorkPos.y));
    ImGui::SetNextWindowSize(ImVec2(G_SIDE_BAR_SIZE, viewport->WorkSize.y));
    ...
    if (ImGui::Begin("SideBar", &always_show,
        ...
        bool content_opened = show_gui["side_content"];
        if (content_opened) { ImGui::PushStyleColor(ImGuiCol_Button, ImGui::GetStyle().Colors[ImGuiCol_ButtonActive]); }
        if (ImGui::Button(content_opened ? ICON_FK_LONG_ARROW_LEFT : ICON_FK_BARS, buttons_size)) { show_gui["side_content"] = !show_gui["side_content"]; }
        ...
        if (ImGui::Button(ICON_FK_OBJECT_UNGROUP, buttons_size)) {
            // TODO :input_side_menu = shape_selected ? I_SIDE_MENU_DEFAULT : I_SIDE_MENU_SHAPE;
        }
        ...
        if (ImGui::Button(ICON_FK_ASTERISK, buttons_size)) {
            // TODO :input_side_menu = spawner_selected ? I_SIDE_MENU_DEFAULT : I_SIDE_MENU_SPAWNER;
        }
        if (spawner_selected) { ImGui::PopStyleColor(); }

        // BOTTOM MENU
        ImGui::SetCursorPos(ImVec2(0.0f, viewport->WorkSize.y - buttons_size.y * 3));
        ...
        ImGui::SetNextWindowPos(ImVec2(viewport->WorkPos.x + buttons_size.x, viewport->WorkPos.y + viewport->WorkSize.y - buttons_size.y * 3));
        ...
        // ZOOM LEVEL
        ImGui::SetNextWindowSize(ImVec2(0.0f, buttons_size.y));
        ImGui::SetNextWindowPos(ImVec2(viewport->WorkPos.x + buttons_size.x, viewport->WorkPos.y + viewport->WorkSize.y - buttons_size.y * 2));
        if (ImGui::BeginPopup("##ZoomLevelWindow", ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar)) {
            ...
            if (ImGui::Button(ICON_FK_SEARCH_MINUS, buttons_size)) {
                // TODO : if (get_camera_size() > 0.1f) { set_camera_size(get_camera_size() + 0.1f); }
            }
            ImGui::SameLine(0.0f, 0.0f);
            float temp_camera_zoom = 100.0f;  // TODO :*(100.0f / get_camera_zoom());
            ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 10.0f);
            ImGui::PushItemWidth(ImGui::GetWindowWidth() * 0.65f);
            if (ImGui::SliderFloat("##SliderFloatZoom", &temp_camera_zoom, 0.1f, 1000.0f, "%.1f%%")) {
                // TODO :set_camera_zoom(100.0f * (100.0f / temp_camera_zoom));
            }
            ImGui::SameLine(0.0f, 0.0f);
            ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 10.0f);
            if (ImGui::Button(ICON_FK_SEARCH_PLUS, buttons_size)) {
                // TODO : if (get_camera_size() < 1000.0f) { set_camera_size(get_camera_size() - 0.1f); }
            }
            ImGui::PopStyleColor();
            ImGui::EndPopup();
        }
    ...
    ImGui::SetNextWindowPos(ImVec2(viewport->WorkPos.x - diff_size + buttons_size.x, viewport->GetCenter().y - center_handle));
    ...
    if (side_content_size == 0.0f) {
        if (ImGui::Begin("##SideWindowHandle", &always_show,
            ...
            if (ImGui::Button(show_gui["side_bar"] ? ICON_FK_CARET_LEFT : ICON_FK_CARET_RIGHT, handle_size)) { show_gui["side_bar"] = !show_gui["side_bar"]; }
            ImGui::End();
        }
    }
