...
void functionnal_Message(const TA& test_result, const TB& true_result) {
    std::cout << "[Functional] " << gmt::to_string(test_result) << " == " << gmt::to_string(true_result) << std::endl;
}

template <typename TA, typename TB>
void error_Message(const TA& test_result, const TB& true_result) {
    std::string delimiter = "\n[-------------------------------[Error]-------------------------------]\n";
    std::cout << delimiter << "Computation Error: " << gmt::to_string(test_result) << " != " << gmt::to_string(true_result) << delimiter << std::endl;
}
...
void assert_Equals(const float& test_result, const float& true_result) {
    if (gmt::decimal_equals(test_result, true_result, 0.05f)) {
        functionnal_Message(test_result, true_result);
    } else {
        error_Message(test_result, true_result);
    }
...
void assert_Equals(const sf::Vector2f& test_result, const sf::Vector2f& true_result) {
    if (gmt::decimal_equals(test_result.x, true_result.x, 0.05f) && gmt::decimal_equals(test_result.y, true_result.y, 0.05f)) {
        functionnal_Message(test_result, true_result);
    } else {
        error_Message(test_result, true_result);
    }
...
/* Test the gmt::Dot(vect2f, vect2f) function */
void test_Dot_vect() {
    ...
    float test_result = gmt::Dot(vect_a, vect_b);
    ...
    test_result = gmt::Dot(vect_a, vect_b);
    true_result = 606.0f;
    assert_Equals(test_result, true_result);
...
/* Test the gmt::remove(int, vect) function */
void test_Remove() {
    ...
    for (int i = 0; i < 5; i++) { vect.emplace_back(std::move(std::make_shared<int>(i))); }
    for (int i = 0; i < 4; i++) { gmt::remove(0, vect); }
    ...
    vect = std::vector<std::shared_ptr<int>>();
    for (int i = 0; i < 5; i++) { vect.emplace_back(std::move(std::make_shared<int>(i))); }
    gmt::remove(1, vect);  // remove pos 1 => {0,2,3,4}
    gmt::remove(3, vect);  // remove pos 3 => {0,2,3}
...
void test_Segment_Projection() {
    ...
    sf::Vector2f test_result = gmt::Segment_Projection({0, 0}, {4, 0}, {5, 3});
    sf::Vector2f true_result = {5, 0};
    assert_Equals(test_result, true_result);

    std::cout << "# Project point [452,-691] on a line [49,-727] [375,-274]: ";
    test_result = gmt::Segment_Projection({49, -727}, {375, -274}, {452, -691});
