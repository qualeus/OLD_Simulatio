...
Vector<T> Vertices<T>::Centroid() const {
    ...
    for (int i = 0; i < vertices_size; i++) {
        ...
        Vector<T> pB = *this->vertices[gmt::modulo(i + 1, vertices_size)];
...
std::vector<std::pair<std::shared_ptr<Vector<T>>, std::shared_ptr<Vector<T>>>> Vertices<T>::Pairs() const {
    ...
    for (int i = 0; i < vertices_size; i++) {
        ...
        std::shared_ptr<Vector<T>> pB = this->vertices[gmt::modulo(i + 1, vertices_size)];
...
std::vector<Vertices<T>> Vertices<T>::Triangulate() const {
    ...
    while (cvertices.vertices.size() >= 3) {
        ...
        for (int i = 0; i < psize; i++) {
            ...
            std::shared_ptr<Vector<T>> pB = cvertices.vertices[gmt::modulo(i + 1, psize)];
            std::shared_ptr<Vector<T>> pC = cvertices.vertices[gmt::modulo(i + 2, psize)];
...
bool Vertices<T>::OrientationShape(const Vertices<T>& vertices) {
    ...
    for (int i = 0; i < vertices_size; i++) {
        ...
        Vector<T> pB = *vertices.vertices[gmt::modulo(i + 1, vertices_size)];
...
bool Vertices<T>::PointInShape(const Vertices<T>& vertices, const Vector<T>& point) {
    gmt::Bounds<T> bounds = vertices.Bounds();

    // The ray of the length is the distance to the farthest horizontal point
    const auto max_x = std::max_element(vertices.vertices.begin(), vertices.vertices.end(), [](const std::shared_ptr<Vector<T>>& lhs, const std::shared_ptr<Vector<T>>& rhs) { return (*lhs).x < (*rhs).x; });
...
std::pair<std::shared_ptr<Vector<T>>, std::shared_ptr<Vector<T>>> Vertices<T>::ClosestEdge(const Vertices<T>& vertices, const Vector<T>& point) {
    ...
    const auto closest = std::min_element(pairs.begin(), pairs.end(), [point](const std::pair<std::shared_ptr<Vector<T>>, std::shared_ptr<Vector<T>>>& lhs, const std::pair<std::shared_ptr<Vector<T>>, std::shared_ptr<Vector<T>>>& rhs) {
